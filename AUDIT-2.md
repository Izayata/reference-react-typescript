...
- **`UserProfile`'s "Personal Data" edit flow is completely unreachable.** A single inverted condition means the only button that enters edit mode is itself hidden until edit mode is already on — see §1.1. `UserProfile.tsx` and all four of its subsections have **0% test coverage** (§4), which is almost certainly why this shipped unnoticed.
...
|---|---|---|---|
| 1.1 | `src/main/components/page/UserProfile/PersonalData/PersonalData.tsx:120` (button) / `:129` (form) | **High** | The view/edit toggle for personal data (firstname/lastname/phone number) is inverted. The data-display block is correctly gated on `{!editMode && (...)}` (line 104), but the "Edit" button that calls `handleEdit()` (which does `setEditMode(true)`) is gated on `{editMode && (...)}` (line 120) — and the edit `<form>` itself is *also* gated on `{editMode && (...)}` (line 129). Since `editMode` starts `false` and the only way to set it `true` is clicking a button that doesn't render until it's already `true`, **there is no way to enter edit mode through the UI at all** — a logged-in user can never edit their name or phone number from the account page. Confirmed pre-existing since the repo's initial commit (`git log --follow` shows no revision ever had the correct condition). The three sibling components in the same directory (`BillingAddress.tsx:115`, `ShippingAddress.tsx:115`, `LoginData.tsx:117`) all correctly use `{!editMode && (<button onClick={handleEdit}>...` — this is the one outlier. |
...
| # | Location | Severity | Description |
|---|---|---|---|
| 4.1 | `src/main/components/page/UserProfile/**` (`UserProfile.tsx`, `BillingAddress.tsx`, `ShippingAddress.tsx`, `PersonalData.tsx`, `LoginData.tsx`) | **High** | Live coverage run: **all five files sit at 0% statements/branches/functions/lines.** This is the entire account/profile-management feature area — one of the app's larger, more state-heavy screens — with zero automated coverage of any kind. This directly explains why §1.1 (the completely unreachable Personal Data edit flow) shipped and has stayed unnoticed: no test ever renders `PersonalData` and asserts that clicking "Edit" reveals the form. |
...
| # | Finding | Severity |
|---|---|---|
| 1.1 | `UserProfile`'s Personal Data edit flow is completely unreachable (inverted `editMode` condition) | **High** |
