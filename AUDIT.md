...
|---|---|---|---|
| 1.1 | `src/main/components/page/Register/Register.tsx:111-112` (`handlePersonalDetailsStep`, catch block — line drifted from `:115-116` after §5.1's unused-import cleanup in this file) | **High — ✅ Fixed** | `handleErrorMessages(e)` is called but its return value (the formatted error string) is discarded instead of being passed to `setModalMessage`. A validation failure on the "personal details" registration step fails with zero user-visible feedback. |
| 1.2 | `src/main/components/page/Register/Register.tsx:135-136` (`handleSetAddressesStep`, catch block — line drifted from `:141-142` after §5.1's unused-import cleanup in this file) | **High — ✅ Fixed** | Same pattern — `handleErrorMessages(e)` result discarded, so address-step validation errors (and any error from the `register()` network call on this step) are silently swallowed. |
...
| 6.8 | `src/main/pages/GalleryPage/index.tsx:35` | **Medium — ✅ Fixed** | An anchor (`<a>`) had no accessible content (`jsx-a11y/anchor-has-content`) — an icon-less link used purely as a CSS `:target`-based slide-navigation dot, with no `aria-label`/visually-hidden text, so a screen reader announced nothing meaningful for it. **Fixed:** added `aria-label={`Ugrás a ${index + 1}. képre`}` ("Go to image N") to each dot, matching the app's existing Hungarian-language `aria-label` convention (e.g. `DisplayShoppingCartContent.tsx`'s "Törlés"/"Növelés"/"Csökkentés"). |
...
| `POST /v1/req/signup` | `POST /v1/registration` | `src/main/components/page/Register/Register.tsx:62` (was `:64` before §5.1's unused-import cleanup in this file) | Wrong path — registration itself is broken. Even if the path were fixed, no CSRF token is attached (§2.2), and the documented endpoint requires one. |
...
| `PUT /v1/customer/updateDefaultShippingAddress` | `PUT /v1/customer/shipping-address` | `src/main/components/page/UserProfile/ShippingAddress/ShippingAddress.tsx:55` (was `:56` before §5.1's unused-import cleanup) | Wrong path (method is correct). |
| `PUT /v1/customer/updatePersonalDetails` | `PATCH /v1/customer/personal-details` | `src/main/components/page/UserProfile/PersonalData/PersonalData.tsx:43` (was `:47` before §5.1's unused-import cleanup) | Wrong path **and** wrong method. |
| `GET /v1/foods/getMenuItemsBy/RESTAURANT` | `GET /v1/foods/menu/{placeToBuy}` | `src/main/components/page/Menu/Menu.tsx:61` (was `:60`; URL built in `src/main/pages/MenuPage/index.tsx:5`) | Wrong path. Also hardcodes the literal `RESTAURANT` segment rather than passing `PlaceToBuyEnum` dynamically — `FANTASY_WORLD` is never reachable from any menu route even once the path is fixed. |
