...
**Date:** 2026-07-26 (last re-verified 2026-07-27 — after the §1, §2, and §8 fixes landed, a pass to correct drifted file:line citations, a follow-up `npm audit fix` once a local npm-cache permission blocker was resolved (surfaced §2.6), and the §3 code-quality/architecture fixes, which surfaced the `CI=true npm run build` finding folded into §5.1)
...
| 3.7 | ~~`.tsx` used for files with no JSX~~ | ~~Low~~ **✅ Fixed** |
