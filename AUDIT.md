...
- ~~**The checkout "Place Order" action and the modal used for every error/success message in the app are not operable by keyboard**.~~ **✅ Fixed** — see §6. The Place Order control is now a real `<button>`; the shared `Modal` has proper dialog semantics, auto-focus, a focus trap, and `Escape`-to-close; the hamburger menu, its full-screen backdrop, and several form-input accessibility issues (invalid `autocomplete`, disorienting `autoFocus`, an unlabelled phone-input field, an empty gallery-slider link) are all fixed too.
- ~~**There is no CI/CD pipeline of any kind**, so none of the above (lint, tests, or vulnerable deps) is caught automatically before it ships.~~ **✅ Fixed** — see §5. `.github/workflows/ci.yml` now runs lint/typecheck/test+coverage/build on every push and PR to `main`, and a `husky`+`lint-staged` pre-commit hook catches lint errors before they're even committed. Getting here required first clearing 126 pre-existing lint errors and the 44 `src/main/**` warnings that were failing `CI=true npm run build` (§5.1) — those last 44 warnings turned out to be entirely `eslint-plugin-jsx-a11y` violations (§5.2), so fixing them for real as part of §6 means **`CI=true npm run build` now succeeds outright** — the workflow's `CI: false` build-step workaround has been removed as no longer needed. `tsconfig.json`'s `target` also now matches the project's `browserslist` (§5.3).
- ~~**No i18n library is present** — all user-facing strings were hardcoded Hungarian literals inline throughout components, notable given the repo's positioning as an industry-reference example.~~ **✅ Fixed** — see §7. `i18next`/`react-i18next` now back every user-facing string in `src/main/**` (503 keys across 18 namespaces), including the `ERR_MSG_*` validation-message constants and the 242-entry phone-country-name lookup, not just a representative slice.
- ~~**The frontend's CSRF handling didn't match the documented contract in two smaller ways**: `/logout` sent its token as a body field instead of the documented `X-CSRF-TOKEN` header, and three CSRF-*exempt* endpoints (`/login`, both password-reset calls) fetched and attached a token anyway.~~ **✅ Fixed** — see §8.3/§8.4. `LogoutButton.tsx` now sends `X-CSRF-TOKEN` via the shared `fetchCsrfToken()` utility; `Login.tsx`/`ForgottenPassword.tsx` no longer fetch or attach a token at all, and their request bodies now match `API_ENDPOINTS.md` exactly (live-verified via real network requests in a browser). With this, **§8 (backend API contract mismatches) is now fully addressed** — every numbered defect (§8.1–§8.4) is fixed; only §8.5, an informational note about backend endpoints with no frontend feature built yet, remains open by design.
...
|---|---|---|---|
| 1.1 | `src/main/components/page/Register/Register.tsx:114-115` (`handlePersonalDetailsStep`, catch block — line drifted from `:115-116` before §5.1's unused-import cleanup, `:111-112` at the last sync, now drifted again after §7 added a `useTranslation` import) | **High — ✅ Fixed** | `handleErrorMessages(e)` is called but its return value (the formatted error string) is discarded instead of being passed to `setModalMessage`. A validation failure on the "personal details" registration step fails with zero user-visible feedback. |
| 1.2 | `src/main/components/page/Register/Register.tsx:139-140` (`handleSetAddressesStep`, catch block — line drifted from `:141-142` before §5.1's unused-import cleanup, `:135-136` at the last sync, now drifted again after §7) | **High — ✅ Fixed** | Same pattern — `handleErrorMessages(e)` result discarded, so address-step validation errors (and any error from the `register()` network call on this step) are silently swallowed. |
| 1.3 | `src/main/components/page/Checkout/Checkout.tsx` (`submitOrder`, outer catch — originally lines 366-367 in the pre-§3.8-split 686-line file, `:353` at the last sync after that split moved/renumbered the surrounding code without changing this logic, now `:355` after §7 added a `useTranslation` import) | **Medium — ✅ Fixed** | Same discarded-return pattern. In practice most real failures are already surfaced by the inner `getOrderToSubmit`/`sendOrderToServer` catches (now `Checkout.tsx:312`, `:341`), but the "order data missing/invalid" case thrown at (now) `:352` reaches only this outer catch and is never shown to the user. |
...
| 2.2 | `src/main/components/page/Register/Register.tsx` (signup `POST`), `src/main/components/page/ShoppingCart/ShoppingCart.tsx:16-22` (`fetchFoodsByIds`, drifted from `:14-20` after §7 added two imports above it), `src/main/utils/myUser/PasswordUtils.ts:16-29` (`isCommonPassword` check — extension and line both corrected, see §8.1) | ~~**Medium**~~ **Resolved / not actually a bug** | These three mutating `fetch` calls send no CSRF token, unlike every other state-changing call in the app. **Final correction, now that §8.1 is fixed and every endpoint has been checked against `API_ENDPOINTS.md`:** only the `Register.tsx` signup case was a real gap — `POST /v1/registration` is documented CSRF-required, and it's now fixed (§8.1) alongside the path correction. `ShoppingCart.tsx`'s and `PasswordUtils.tsx`'s calls were never actually bugs: `POST /v1/foods/cart` and `POST /v1/registration/common-password` are both documented **CSRF-exempt**, so sending no token there is correct, not inconsistent. |
...
| 3.4 | `src/main/components/page/Checkout/Checkout.tsx:309-311,364`, `Register.tsx:127`, `src/main/utils/pages/account/accountPageUtils.ts:5` (extension corrected — this citation had said `.tsx`, but the file was already `.ts` even before this finding was fixed), `DisplayShoppingCartContent.tsx:102-103`, `UserProfile/LoginData/LoginData.tsx:91` | **Low — ✅ Fixed** | Leftover debug `console.log`s, including one that logs the full constructed order object and payment-method flags during checkout (`Checkout.tsx:364`). Several more commented-out `console.log`/JSX blocks exist (`App.tsx:24,96`, `FoodDetails.tsx:4,6,75,82`, `fetchCsrfToken.tsx:16,18`, `LogoutButton.tsx:29-30`). **Fixed:** all of the above removed, plus the same pattern in `PhoneNumberUtils.ts` (extension corrected, same reason as `accountPageUtils.ts` above; not explicitly cited but same category) and a dead orphaned `Page3`/`Order/index.tsx` placeholder component and its commented-out route in `App.tsx`. All of this describes already-removed code, so the cited line numbers are historical pointers to where the debug statements used to be, not current content. |
| 3.5 | `src/main/converter/AddressModelConverter.ts:9-61` (extension corrected — this citation had said `.tsx`, but the file was already `.ts` even before this finding was fixed) | **Low — ✅ Fixed** | Four near-identical functions (differ only by a `shipping`/`billing` field-name prefix) — a small shared `buildAddress(prefix, data)` helper would remove the duplication. More broadly: the Model → Builder → Converter → Utils pattern is applied uniformly for composite models, but for a single optional-string wrapper like `FloorDoorModel` (23 lines) it still generates a model + decorator stack + a 22-line `FloorDoorUtils.ts` (extension corrected, same reason) that duplicates the model's own validation constants — worth reconsidering per-model boilerplate cost for the simplest leaf types (note: `FloorDoorModel` has no dedicated Builder, so the pattern already isn't applied with full uniformity). **Fixed:** the four functions now share a single `buildAddress(fields: AddressFields)` helper; each caller extracts its own properly-typed fields and delegates. The broader per-model boilerplate observation (leaf-type cost) is a design tradeoff, not a defect, and was left as-is. |
...
| # | Location | Severity | Description |
|---|---|---|---|
| 6.1 | `src/main/components/functional/Modal/Modal.tsx:11-19` | **High — ✅ Fixed** | Used for every error/success message app-wide. Missing `role="dialog"`, `aria-modal="true"`, and `aria-labelledby`; no focus trap, no auto-focus on open, and no `Escape`-to-close — a keyboard-only user cannot dismiss it except by tabbing to the one visible "OK" button. **Fixed:** added `role="dialog"`, `aria-modal="true"`, and `aria-labelledby` pointing at the message `<p>`'s new `id`. The OK button (the modal's only focusable element) now receives focus on mount via a `useRef` + `useEffect`, `Escape` calls `onClose`, and `Tab` is intercepted to keep focus on that same button — a full trap, since there's only ever one focusable element in this modal. Zero prior test coverage on this component; added `src/test/components/functional/Modal/Modal.test.tsx` (4 tests: dialog semantics + auto-focus, OK-click calls `onClose`, `Escape` calls `onClose`, `Tab` keeps focus trapped) to cover the new behavior. Manually verified in a real browser: dialog opens focused, `Escape` closes it, visual appearance unchanged. |
| 6.2 | `src/main/components/page/Checkout/CheckoutOrderSummarySection.tsx:132` (was `:134`, `:131` at the last sync, now drifted again after §7 added a `useTranslation` import) | **High — ✅ Fixed** | The "Megrendel" (Place Order) control — the single most important call-to-action in the checkout flow — was `<div className='...' onClick={onSubmitOrder}>`, not a `<button>`, with no `role`, `tabIndex`, or `onKeyDown`. It was unreachable via keyboard. Inconsistent with `DisplayShoppingCartContent.tsx:155-195` (drifted from `:153-193` after §7 added a `useTranslation` import + hook line) in the same feature area, which correctly uses `<button aria-label>` for quantity/delete controls. **Fixed:** changed to `<button type='button' className='...' onClick={onSubmitOrder}>` — same CSS class (already fully self-styled, no reliance on default button chrome, and already used as a real `<button>` elsewhere in the app, e.g. `RegisterFormUserDetails.tsx`), so the visual appearance is unchanged but it's now natively keyboard-operable (`Tab` + `Enter`/`Space`). Manually verified in a real browser. |
| 6.3 | `src/main/components/navigation-bar/components/hamburger-menu/button/HamburgerMenuButton.tsx:31-38` (was `:28-35`, drifted after §7 added a `useTranslation` import + hook line) | **Medium — ✅ Fixed** | The menu toggle set `aria-expanded`/`aria-controls`/`aria-hidden` but was rendered as a plain clickable `<FontAwesomeIcon>` (SVG) with no `role="button"`, `tabIndex`, `onKeyDown`, or `aria-label` — partially ARIA-wired but not actually operable or announced correctly. **Fixed:** added `role="button"`, `tabIndex={0}`, an `onKeyDown` handler activating on Enter/Space, and an `aria-label` (originally the hardcoded literal `"Menü megnyitása"`, since migrated to `t('nav.openMenuAriaLabel')` by §7's i18n extraction — same rendered text). **Caught during manual verification, not by lint:** the first attempt also dropped the existing `aria-hidden="false"` as seemingly-redundant with the new `role`/`aria-label` — but `FontAwesomeIcon` defaults to `aria-hidden="true"` when that prop isn't explicitly set, which silently made the icon unfocusable again despite `tabIndex={0}` being present in the DOM. Real browser testing (Tab-cycling to the element, not just reading the JSX) caught this; static lint/type-checking did not. Re-added `aria-hidden="false"` explicitly. This hamburger button only renders/matters below a `min-width: 1280px` CSS breakpoint (desktop shows an always-visible nav bar instead, per `hamburger-menu.css`), so verification used a mobile (390×844) viewport — Playwright's default 1280×720 viewport sits exactly on that breakpoint and would have shown the desktop nav instead, hiding this control entirely. |
| 6.4 | `src/main/components/navigation-bar/navigation-menu/NavigationMenu.tsx:38` (backdrop; was `:24` before the Escape-handler added below shifted it; untouched since, still accurate), `src/main/components/navigation-bar/components/shopping-bag/dropdown/ShoppingBagDropdown.tsx:15` (bubble-catcher; was `:8`, then `:13` at the last sync, now drifted again after §7 added a `useTranslation` import) | **Low — ✅ Fixed** | Additional clickable `<div onClick>` wrappers with no keyboard/ARIA affordances, same shape as 6.2/6.3. **Fixed, but not identically** — the two turned out to be different patterns on inspection: `NavigationMenu.tsx`'s div is a genuine full-viewport modal backdrop (confirmed via `menu-container.css`: `position: fixed; width: 100%; height: 100vh` when open) — added a real `Escape`-to-close handler (`document`-level `keydown` listener while the menu is open) as the correct keyboard equivalent, and suppressed the two specific jsx-a11y rules on that line with a comment, since making an invisible full-screen backdrop a tab stop would be worse UX, not better. `ShoppingBagDropdown.tsx`'s div only wraps two real `<NavLink>` elements and its `onClick` is purely a bubble-catcher that closes the dropdown after a link is clicked (bubbles from real click **or** keyboard-triggered Enter activation, since browsers dispatch a real bubbling `click` event either way) — the links are already fully keyboard-operable on their own, so this one is also suppressed with an explanatory comment rather than given a redundant fake `role="button"`. |
| 6.5 | `src/main/components/input/customer/PhoneNumberInput/PhoneNumberInput.tsx:40` (was `:38`, drifted after §7 added a `useTranslation` import) | **Medium — ✅ Fixed** | Discovered when §5.2 turned on `eslint-plugin-jsx-a11y`: the `<label>` wrapping the third-party `PhoneInput` widget wasn't recognized as associated with a control (`jsx-a11y/label-has-associated-control`) — the widget renders its own internal `<input>` not directly nested in a way the rule can trace, so a screen reader may not have announced "Telefonszám:" when the phone field was focused. **Fixed:** added `id: 'phone-number-input'` to the widget's `inputProps` (passed straight through to the real underlying `<input>`) and `htmlFor="phone-number-input"` on the `<label>` — a real association, not just a lint workaround — plus registered `PhoneInput` in `.eslintrc.json`'s `label-has-associated-control` rule options (`controlComponents: ["PhoneInput"]`) so the linter can verify this pattern for third-party form-control components going forward. |
| 6.6 | `src/main/components/input/myUser/PasswordInput/PasswordInput.tsx:63` | **Low — ✅ Fixed** | `autoComplete="password"` wasn't a valid HTML autocomplete token (`jsx-a11y/autocomplete-valid`) — the valid values are `current-password`/`new-password`; as-is, browsers/password managers may not have autofilled this field correctly. **Fixed:** `PasswordInput` is shared across 5 call sites with different semantic roles (registration/reset "new password", login-data "current password" + "new password" + "confirm new password") — rather than hardcoding one value, `autoComplete` is now derived from the existing `name` prop (`current-password` when `name === 'currentPassword'`, `new-password` otherwise), correctly handling all 5 usages without changing any call site. |
| 6.7 | `src/main/components/input/myUser/UsernameInput/UsernameInput.tsx:71` | **Low — ✅ Fixed** | The username field had `autoFocus` (`jsx-a11y/no-autofocus`) — automatically stealing focus on page load is disorienting for screen-reader users, who lose their place in the page. **Fixed:** removed the `autoFocus` prop entirely. |
| 6.8 | `src/main/pages/GalleryPage/index.tsx:37` (was `:35`, drifted after §7 added a `useTranslation` import) | **Medium — ✅ Fixed** | An anchor (`<a>`) had no accessible content (`jsx-a11y/anchor-has-content`) — an icon-less link used purely as a CSS `:target`-based slide-navigation dot, with no `aria-label`/visually-hidden text, so a screen reader announced nothing meaningful for it. **Fixed:** added an `aria-label` ("Go to image N") to each dot, matching the app's existing Hungarian-language `aria-label` convention (e.g. `DisplayShoppingCartContent.tsx`'s "Törlés"/"Növelés"/"Csökkentés"). Originally a hardcoded template literal (`` `Ugrás a ${index + 1}. képre` ``); since migrated by §7's i18n extraction to `t('gallery.goToImageAriaLabel', { index: index + 1 })` — same rendered text, now routed through `i18next` interpolation instead of being inlined. |

**Fixed:** all eight findings addressed together as one pass. As a direct consequence of fixing 6.2–6.8 (all `eslint-plugin-jsx-a11y` violations, §5.2), `npm run lint` no longer needs any of the 6 rule-severity downgrades §5.2 introduced as a temporary measure — `.eslintrc.json` now only has `label-has-associated-control`'s legitimate `controlComponents` option (needed for the third-party `PhoneInput`, not a severity change), with every other jsx-a11y rule at its full default `"error"`. Since the 44 `src/main/**` warnings that had been failing `CI=true npm run build` (§5.1) are now also all gone (the last of them were these jsx-a11y violations), **`CI=true npm run build` now succeeds for the first time in this audit's history** — the `.github/workflows/ci.yml` build step's `CI: false` override (added in §5.1 specifically to work around this warning debt) has been removed, since the condition that motivated it no longer holds; the workflow now runs the standard `CI=true npm run build`, so any *new* `src/main/**` warning will fail CI immediately going forward instead of silently accumulating. Live-verified after all changes: `npx tsc --noEmit` passes with zero errors; `CI=true npm test -- --watchAll=false` passes all 42 suites (461 tests, up from 41/457 — the 4 new `Modal.test.tsx` tests); `npm run test:coverage` passes (47.34/31.68/44.72/48.38, still above the §4.2 threshold); `npm run lint` exits `0` with 182 problems (0 errors, 182 warnings — all in `src/test/**`'s exhaustive model/builder tests, none jsx-a11y-related); `CI=true npm run build` exits `0`, "Compiled successfully". Manually verified in a real browser (dev server + Playwright, both desktop and a 390×844 mobile viewport for the hamburger menu specifically): Modal dialog semantics/focus/Escape, Checkout submit button and payment checkboxes, and hamburger-menu keyboard open/close all work as intended, with no new console errors beyond the already-documented pre-existing ones.

### 7. Portability / localization — ✅ Fixed

| # | Location | Severity | Description |
|---|---|---|---|
| 7.1 | repo-wide | **Low — ✅ Fixed** | No i18n library is present (`package.json` has no `react-i18next`/`i18next`/`react-intl` etc.). All user-facing strings — labels, error messages, page copy — are hardcoded Hungarian literals inline in components (e.g. `Login.tsx`, `UserProfile.tsx`, `ErrorUtils.tsx`-driven messages throughout). Not a bug, but worth flagging given this repo is also positioned as an industry-reference example: a reference implementation would typically demonstrate an extraction/i18n layer. |

**Fixed:** given the scale difference between a representative-slice demo and a full migration, the user was asked and chose full extraction — every hardcoded Hungarian (and locale-dependent) user-facing string in `src/main/**` is now routed through `i18next`/`react-i18next`, not just a sample. `i18next@23.16.8` + `react-i18next@14.1.3` were added (pinned below the `18.x`/`17.x` lines specifically because this project's `typescript@4.9.5` doesn't satisfy their `typescript: '^5 || ^6 || ^7'` peer requirement). `src/main/i18n/i18n.ts` configures a single `hu` locale with one combined `translation` namespace (organized internally via nested JSON keys per domain/component, e.g. `errors.*`, `checkout.*`, `userProfile.*`) backed by `src/main/i18n/locales/hu/hu.json`, which now holds 503 keys across 18 namespaces — including the 242-entry `phoneCountryNames` lookup migrated out of `PhoneNumberUtils.ts` (previously a hardcoded country-code→Hungarian-name object feeding `react-phone-input-2`'s `localization` prop, now loaded via `i18n.t('phoneCountryNames', { returnObjects: true })`).

Two call patterns are used, chosen by where the string is evaluated: inside function components, `useTranslation()`'s `t()` is used normally. But `class-validator` decorators (e.g. `@NotBlank({ message: ERR_MSG_X })`, used throughout `src/main/model/**` and referenced from `src/main/utils/**`'s `ERR_MSG_*` constants) evaluate their config object at **module-import time**, before any component tree — or `useTranslation()` — exists. Those ~97 `ERR_MSG_*` constants (across 26 `utils/**` files) and a couple of directly-decorated model fields (e.g. `ResetPasswordRequestModel.tsx`) instead call `i18n.t()` directly on the imported singleton, which only works because i18next is guaranteed to already be initialized: `src/index.tsx` imports `./main/i18n/i18n` as its literal first line (before anything that transitively imports an `ERR_MSG_*` file), and `src/setupTests.ts` does the same for Jest, since individual test files import components directly rather than through `index.tsx` and would otherwise render with no i18next instance registered.

Deliberately left untranslated: proper nouns and brand names (`ImagineBar`, `Facebook`/`Instagram`/`Tiktok`/`Youtube`), the footer's "Credits"/developer-attribution/copyright line, and image `alt`/attribution text that's already in English — translating those would be nonsensical regardless of locale, and isn't what this finding was about (Hungarian-locale lock-in, not "every string must route through i18next unconditionally").

Live-verified after the full migration: `npx tsc --noEmit` passes with zero errors; `CI=true npm test -- --watchAll=false` passes all 42 suites (461 tests, unchanged from before this fix — every `ERR_MSG_*`/JSX-text change was verified against the *existing* test suite as a correctness oracle, since tests assert against the imported constant references rather than duplicating the literal strings); `npm run test:coverage` passes (47.65/31.68/44.72/48.68, still above the §4.2 threshold); `npm run lint` exits `0` with 182 problems (0 errors, same pre-existing `src/test/**` `no-explicit-any` warnings as before, none new); `CI=true npm run build` exits `0`, "Compiled successfully" (bundle grew ~19.7 kB gzipped, expected given `i18next`/`react-i18next` are new runtime dependencies). Manually verified in a real browser (dev server + Playwright): Home, Menu, Allergen, Gallery, Login, Register, ForgottenPassword, ShoppingCart, and Checkout all render the same Hungarian copy as before with correct diacritics, no `react-i18next`/`missingKey` console warnings, and no raw translation-key literals leaking into the rendered DOM.

### 8. Backend API contract mismatches — ✅ Fixed
...
|---|---|---|---|
| `GET /v1/req/isEmailExist/{email}` | `GET /v1/registration/email/{email}/exists` | `src/main/utils/EmailUtils.tsx:17` (line drifted after the §7 i18n migration added an `import i18n from '../i18n/i18n'` above it) | Wrong path. |
| `GET /v1/req/isUsernameExist/{username}` | `GET /v1/registration/username/{username}/exists` | `src/main/utils/myUser/UsernameUtils.ts:16` (extension and line both corrected — this citation had said `.tsx` since before this doc's last sync, even though the file was already renamed `.tsx`→`.ts` back in §3.7) | Wrong path. |
| `POST /v1/req/isCommonPassword` | `POST /v1/registration/common-password` | `src/main/utils/myUser/PasswordUtils.ts:16-29` (extension/line corrected, same as above) | Wrong path — but `checkPasswordIsCommon` isn't called from anywhere else in `src/`, so this one's unreachable regardless (dead code, see also §3). |
| `POST /v1/req/signup` | `POST /v1/registration` | `src/main/components/page/Register/Register.tsx:64` (drifted again after §7 added a `useTranslation` import — was `:62` at the last sync, `:64` before that too, before §5.1's unused-import cleanup; the two changes happened to net out to the same number) | Wrong path — registration itself is broken. Even if the path were fixed, no CSRF token is attached (§2.2), and the documented endpoint requires one. |
| `GET /v1/account/getAuthenticatedUser` | `GET /v1/account/me` | `src/main/utils/pages/account/accountPageUtils.ts:4` (extension/line corrected, same as `UsernameUtils.ts` above) | Wrong path — the account page can't load the logged-in user's data. |
| `PUT /v1/account/updatePassword` | `PATCH /v1/account/password` | `src/main/components/page/UserProfile/LoginData/LoginData.tsx:48` (drifted after §7 added an import + a `useTranslation()` hook line above it) | Wrong path **and** wrong method. |
| `PUT /v1/customer/updateBillingAddress` | `PUT /v1/customer/billing-address` | `src/main/components/page/UserProfile/BillingAddress/BillingAddress.tsx:57` (drifted after §7, same reason) | Wrong path (method is correct). |
| `PUT /v1/customer/updateDefaultShippingAddress` | `PUT /v1/customer/shipping-address` | `src/main/components/page/UserProfile/ShippingAddress/ShippingAddress.tsx:57` (was `:56` before §5.1's unused-import cleanup, `:55` at the last sync, now drifted again after §7) | Wrong path (method is correct). |
| `PUT /v1/customer/updatePersonalDetails` | `PATCH /v1/customer/personal-details` | `src/main/components/page/UserProfile/PersonalData/PersonalData.tsx:45` (was `:47` before §5.1's unused-import cleanup, `:43` at the last sync, now drifted again after §7) | Wrong path **and** wrong method. |
| `GET /v1/foods/getMenuItemsBy/RESTAURANT` | `GET /v1/foods/menu/{placeToBuy}` | `src/main/components/page/Menu/Menu.tsx:63` (was `:60`, then `:61` at the last sync, now drifted again after §7 added two imports; URL built in `src/main/pages/MenuPage/index.tsx:5`, unchanged) | Wrong path. Also hardcodes the literal `RESTAURANT` segment rather than passing `PlaceToBuyEnum` dynamically — `FANTASY_WORLD` is never reachable from any menu route even once the path is fixed. |
| `POST /v1/foods/getShoppingCartItemsData` | `POST /v1/foods/cart` | `src/main/components/page/ShoppingCart/ShoppingCart.tsx:16` (drifted after §7 added two imports above `fetchFoodsByIds`) | Wrong path — the cart page can't resolve line-item names/prices/images. |
...
| `GET /v1/allergen/getAllAllergens` | `GET /v1/allergens` | `src/main/utils/food/AllergenUtils.ts:7-13` (extension/line corrected, same reason as `UsernameUtils.ts` above) | Wrong path (singular `allergen` vs. documented plural `allergens`) — the allergens page 404s. |
| `POST /v1/orders/createOrder` | `POST /v1/orders` (authenticated) or `POST /v1/orders/guest` (public) | `src/main/components/page/Checkout/Checkout.tsx:323` (was `:333` before the §3.8 split, `:321` at the last sync, now drifted again after §7 added a `useTranslation` import) | Wrong path either way — order submission, the entire commercial point of the app, 404s. See also §8.2. |
...
#### 8.3 CSRF tokens sent to endpoints the backend documents as CSRF-exempt — **Low — ✅ Fixed**
...
**Fixed:** all three calls no longer fetch or attach a CSRF token. `Login.tsx`'s `/login` `POST` now sends only `username=...&password=...` (no `_csrf` field), matching the documented `application/x-www-form-urlencoded` body exactly. `ForgottenPassword.tsx`'s two calls (`request-password-reset-link`, `set-new-password`) no longer set an `X-CSRF-TOKEN` header, and now send only `Content-Type: application/json` with the documented JSON body. Live-verified via a real browser + Playwright, inspecting the actual network requests: no `GET /csrf-token` request fires before any of the three calls, `POST /login`'s body is exactly `username=testuser&password=testpass`, and `POST /v1/password-reset/request-password-reset-link`'s body is exactly `{"email":{"value":"..."},"myUsername":{"value":"..."}}` with no CSRF header — both matching `API_ENDPOINTS.md` byte-for-byte.

#### 8.4 `/logout`'s CSRF token is sent as a body field, not a header — **Medium — ✅ Fixed**

`API_ENDPOINTS.md` describes CSRF as "send `X-CSRF-TOKEN`" for any non-exempt mutating call, and `POST /logout` is explicitly listed as CSRF-required. `LogoutButton.tsx:20-38` (was `:17-34`, drifted after §7 added a `useTranslation` import + hook line — this is the fixed code's current location, not a historical pointer) does fetch and send a token, but as a `_csrf` field inside a `URLSearchParams` body (same pattern as `Login.tsx`, where it's harmless because `/login` is exempt) — not as the `X-CSRF-TOKEN` header the doc describes elsewhere. Whether Spring's `CsrfFilter` accepts a body parameter alongside/instead of the header depends on the backend's `CsrfTokenRequestHandler` configuration, which isn't visible from the frontend — flagged as a likely-but-unconfirmed logout failure, worth a quick manual check against the real backend rather than assuming either way.
...
`POST /v1/foods` (authenticated menu-item creation), `GET /v1/customers` (admin customer list), `GET /v1/orders/{id}` (order detail/receipt lookup), and `GET /v1/ingredients` are all defined in `API_ENDPOINTS.md` but never called anywhere in `src/`. Not a bug — there's simply no admin UI, order-history/receipt view, or ingredient listing built yet — but worth noting for anyone using this repo as a reference: the backend's capability surface is larger than what the current frontend exposes. Not fixed, and deliberately not treated as an action item: per its own severity tag, this is a non-defect observation, and "fixing" it would mean building four entirely new features (admin UI, customer list, order history/receipt view, ingredient listing) — real feature work far outside the scope of a contract-mismatch audit.

**Fixed:** all four numbered defects in this section (§8.1, §8.2, §8.3, §8.4) are now fixed; §8.5 remains open by design, as noted above (not a defect). Live-verified after the §8.3 fix: `npx tsc --noEmit` passes with zero errors; `CI=true npm test -- --watchAll=false` passes all 42 suites (461 tests, unchanged); `npm run lint` exits `0` with 182 problems (0 errors, same pre-existing warnings); `CI=true npm run build` exits `0`, "Compiled successfully" (bundle shrank 10 B gzipped from removing the three now-unnecessary CSRF-token fetches/headers).
...
| 6.1 | ~~Modal has no keyboard/ARIA support~~ | ~~High~~ **✅ Fixed** |
| 6.2 | ~~Checkout "Place Order" is an unreachable-by-keyboard `<div>`~~ | ~~High~~ **✅ Fixed** |
| 6.5 | ~~`PhoneNumberInput` label not associated with its control~~ (found via §5.2) | ~~Medium~~ **✅ Fixed** |
| 6.8 | ~~`GalleryPage` anchor has no accessible content~~ (found via §5.2) | ~~Medium~~ **✅ Fixed** |
...
| 5.2 | ~~No `jsx-a11y` lint rules configured~~ | ~~Medium~~ **✅ Fixed** — surfaced 16 real violations (4 new); all fixed for real in §6, no rule downgrades remain |
...
| 6.3 | ~~Hamburger menu toggle not keyboard-operable~~ | ~~Medium~~ **✅ Fixed** |
...
| 6.4 | ~~Additional non-semantic clickable `<div>`s~~ | ~~Low~~ **✅ Fixed** |
| 6.6 | ~~`PasswordInput` malformed `autocomplete` value~~ (found via §5.2) | ~~Low~~ **✅ Fixed** |
| 6.7 | ~~`UsernameInput` uses `autoFocus`, disorienting for screen readers~~ (found via §5.2) | ~~Low~~ **✅ Fixed** |
| 7.1 | ~~No i18n layer; all UI text hardcoded in Hungarian~~ | ~~Low~~ **✅ Fixed** — full `i18next`/`react-i18next` extraction, 503 keys across 18 namespaces |
| 8.3 | ~~Unneeded CSRF token fetches on 3 documented CSRF-exempt endpoints~~ | ~~Low~~ **✅ Fixed** |
...
$ npm run lint   # after the §6 accessibility fixes (which fixed §5.2's jsx-a11y violations for real)
✖ 182 problems (0 errors, 182 warnings)   # -16 vs. after §5.2; all remaining warnings are in
                                            # src/test/**, none jsx-a11y-related
(process exit code: 0)

$ npx tsc --noEmit   # after the §6 fixes
(no output, exit code 0)

$ CI=true npm test -- --watchAll=false   # after the §6 fixes
Test Suites: 42 passed, 42 total
Tests:       461 passed, 461 total
(process exit code: 0; 41→42 suites and 457→461 tests is the new Modal.test.tsx,
 4 tests covering the dialog semantics/focus-trap/Escape behavior added in 6.1)

$ npm run test:coverage   # after the §6 fixes
All files | 47.34% Stmts | 31.68% Branch | 44.72% Funcs | 48.38% Lines
(process exit code: 0 — still passes the §4.2 threshold)

$ CI=true npm run build   # after the §6 fixes — the 44 src/main/** warnings blocking this
                           # since §5.1 were entirely the jsx-a11y violations fixed here
Compiled successfully.
(process exit code: 0 — first time this has ever passed in this audit's history;
 .github/workflows/ci.yml's build step no longer needs the CI:false workaround
 and now runs the standard `CI=true npm run build`)

$ npx tsc --noEmit   # after the §7 i18n/localization fix (full i18next/react-i18next extraction)
(no output, exit code 0)

$ CI=true npm test -- --watchAll=false   # after the §7 fix
Test Suites: 42 passed, 42 total
Tests:       461 passed, 461 total
(process exit code: 0; unchanged from §6 — every ERR_MSG_*/JSX-text change was verified
 against the existing suite as a correctness oracle, since tests assert against imported
 constant references rather than duplicated literal strings)

$ npm run test:coverage   # after the §7 fix
All files | 47.65% Stmts | 31.68% Branch | 44.72% Funcs | 48.68% Lines
(process exit code: 0 — still passes the §4.2 threshold)

$ npm run lint   # after the §7 fix
✖ 182 problems (0 errors, 182 warnings)   # unchanged — zero net new lint debt from the
                                            # i18n migration
(process exit code: 0)

$ CI=true npm run build   # after the §7 fix
Compiled successfully.
(process exit code: 0; bundle +19.72 kB gzipped — expected, i18next/react-i18next are
 new runtime dependencies)

$ npx tsc --noEmit   # after the §8.3 CSRF-exempt-endpoint fix
(no output, exit code 0)

$ CI=true npm test -- --watchAll=false   # after the §8.3 fix
Test Suites: 42 passed, 42 total
Tests:       461 passed, 461 total
(process exit code: 0; unchanged from §7)

$ npm run lint   # after the §8.3 fix
✖ 182 problems (0 errors, 182 warnings)   # unchanged
(process exit code: 0)

$ CI=true npm run build   # after the §8.3 fix — every numbered defect in §8 is now fixed
Compiled successfully.
(process exit code: 0; bundle -10 B gzipped from removing the three now-unnecessary
 CSRF-token fetches/headers)

$ npm audit   # re-verified during this doc-sync pass, unchanged since §2.4/§2.6
73 vulnerabilities (4 low, 6 moderate, 63 high, 0 critical)
```
